...
public:
    ...
    [[nodiscard]] T norm() const requires NormalizableScalar<T> {
        return geometry::length<T, N>(data);
    }

    [[nodiscard]] Vector normalized() const requires NormalizableScalar<T> {
...
template <Scalar T> using Vec2 = Vector<T, 2>;
template <Scalar T> using Vec3 = Vector<T, 3>;
template <Scalar T> using Quat = Vector<T, 4>;
